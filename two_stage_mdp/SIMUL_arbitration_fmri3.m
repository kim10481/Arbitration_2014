function [output_info]=SIMUL_arbitration_fmri3(PART_NUMBER, SESS_NUMBER, session_opt, COLOR_CASE, test_switch)
% [NOTE] the goal is changing in each trial.
% (ex) SIMUL_arbitration_fmri('john',1,'pre')
% (ex) SIMUL_arbitration_fmri('john',1,'fmri')

% testtest
EXP_NAME_IN = num2str(PART_NUMBER);
...
switch test_switch;
    ...
    case 'test'
        ...
        if(strcmp(session_opt,'pre')==1)
            Tot_block=4; % # of total blocks (MUST be the multitude of 4(=#of conditions)
        end
    ...
    case 'real'
        ...
        % fmri-session : # of blocks
        if(strcmp(session_opt,'fmri')==1)
            Tot_block=56; % 16; % # of total blocks (MUST be the multitude of 4(=#of conditions)
        end
...
if(strcmp(session_opt,'pre')==1) % pre-session
    ffw_speed=1;%4; % fast forward speed, 1: normal speed
end
...
[myMap N_state N_action N_transition]=Model_Map_Init2('sangwan2012c',map_opt);
% create my state
...
DrawFormattedText(wPtr, 'The experiment will start soon...\n(Please press any key to start, when instructed)', 'center', 'center');
Screen('Flip', wPtr);  
