function [myArbitrator]=DirichletProcessNormalMix_Arbitration_Init(N_state,N_action,N_transition, myMap, data_in)
%% NAIVE model (distance based)
% weight 관련해서도 바꿔야함.

myArbitrator.T = 10;

myArbitrator.K1 = size(data_in{1,1}.DPNMM_MODEL.SPE, 1); % number of clusters of SPE-IGMM
myArbitrator.K2 = size(data_in{1,1}.DPNMM_MODEL.RPE, 1); % number of clusters of RPE-IGMM
...
myArbitrator.zspe = zeros(1,10);
myArbitrator.zspe_bl = 0;
myArbitrator.zrpe = zeros(1,10);
myArbitrator.zrpe_bl = 0;
