...
save_path_neuroecon=[pwd '\..\regressors_contrasts\'];%'\\143.248.30.94\bmlsamba\swlee\repro_fmri\fmri_arbitration\regressors_contrasts\';
...
TYPE_REGRESSOR=[1 1, 1.5 1.5, 1.5 1.5, 1.5 1.5, 2 2, 1.5 1.5 1.5 1.5 1.5 1.5 1.5, 3 3 3 3 1.5 3 3 3 3 3 3 1 1 1 1]; % 1: parametric modulation (0-duration), 1.5:parmetric modulation (non-zero duration), 1.7:parametric modulation (with decision onset)  2: extra continuous parametric modulation (TR-fixed) - this will be used by "dummy" regressor. 3 : IDK
...
row_mat=[7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7]; % from which row in the SBJ{}.regressor matrix the signal needs to be extracted. e.g., uncertainty of 0 prediction error
...
param_regressor_type_cue={'SPE', 'RPE','uncertaintyM1','uncertaintyM2','MAXinvFano12','Qsarsa','Qfwd','Qarb', 'dQbwdEnergy'};
...
if option_best_param_load == 1
    ...
    for i = 1:1:num_sbj_included
        SBJ_list = dir([seed_path_result '\result_simul\SBJ_' LIST_SBJ{1,ind_sbj_included(i)} '*']);
...
%     SBJ=SBJ_keep;
    % [1] model optimization
    mode.out=1;
    myFunc_bu = @(x) eval_ArbitrationRL3c(x, SBJ2, mode); % define a new anonymous function
    [model_BayesArb.param, model_BayesArb.val]=fminsearchbnd(myFunc_bu, param_init, param_BoundL, param_BoundU, optimset('Display','iter','MaxIter',mode.max_iter));   % X0,LB,UB
    % [2-1] add regressor vector to SBJ
    mode.out=99;
    SBJ2=eval_ArbitrationRL3c(model_BayesArb.param,SBJ2,mode);
    % [3] save
    model_BayesArb.mode=mode;
...
if(option_optimizing_model==2)
    ...
    for ff=1:1:length(list_sbj_included)
        ...
        SBJ0=eval_ArbitrationRL3c(model_BayesArb.param,SBJ0,mode); % refresh and add the regressor part
        SBJ1{1,ff}=SBJ0{1,1};
    end
    SBJ=SBJ1;
